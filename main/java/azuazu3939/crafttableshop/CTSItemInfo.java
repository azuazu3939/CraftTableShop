...
public class CTSItemInfo {
    ...
    public static ItemStack ItemReturner(String itemString) {
        ...
        if (!(CraftTableShop.getInstance().getSetUpConfig().getConfig().isSet(itemString))) return null;

        for (MythicItem mythicItem : MythicMobs.inst().getItemManager().getItems()) {

            if (Objects.equals(CraftTableShop.getInstance().getSetUpConfig().getConfig().getString(itemString), mythicItem.getInternalName())) {
